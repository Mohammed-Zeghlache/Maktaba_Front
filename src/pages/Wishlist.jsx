...
import { useAuth } from '../context/AuthContext';
import { showToast } from '../components/Toast';
import { apiCall, bookTitle } from '../services/api';
...
function Wishlist() {
  ...
  return (
    <div className="container" style={{ padding: '20px 0 40px' }}>
      <h1>{lang === 'ar' ? 'المفضلة' : 'Wishlist'} ({books.length})</h1>
      
      {loading && <p>Loading...</p>}
      
      {!loading && books.length === 0 && (
        <div>
          <p>{lang === 'ar' ? 'لا توجد كتب في المفضلة' : 'No books in wishlist'}</p>
          <button className="btn btn-gold" onClick={() => navigate('/browse')}>
            {lang === 'ar' ? 'استعراض الكتب' : 'Browse Books'}
          </button>
        </div>
