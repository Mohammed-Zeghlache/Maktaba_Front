...
function Wishlist() {
  ...
  const fetchWishlist = async () => {
    ...
    try {
      ...
      if (bookList.length === 0) {
        console.log('⚠️ No books in wishlist');
      }
    } catch (error) {
  ...
  return (
    <div className="container" style={{ padding: '20px 0' }}>
      ...
      {error && (
        <div style={{ color: 'red', padding: '10px', background: '#ffeeee', borderRadius: '8px' }}>
          Error: {error}
        </div>
      )}
      ...
      {!loading && !error && books.length === 0 && (
        <p>No books in wishlist. Add some!</p>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px' }}>
          {books.map(book => (
            <div 
              ...
              onClick={() => navigate(`/book/${book.id}`)}
            >
              <h3>{book.title_ar || book.title_en || 'No title'}</h3>
              <p>ID: {book.id}</p>
